...
## パイプライン処理
### julia標準のパイプライン処理
ここではそもそもパイプライン処理とは何ぞやから解説したいと思います。
簡単な例として、次のような関数が入れ子になった計算をしたいとします。
...
このような記法を採用するメリットは、可読性が高まるだけでなく、実行結果を確認しながら関数を追加できることです。

### 引数が複数ある場合の対応
ところが次のように、複数の引数をとる関数をパイプライン処理で実装したいときは、
そのままでは実行できないので、無名関数を使う必要がでてきます。

```
add(a,b) = a+b
1 |> (x -> add(x,2))
```
しかし、このような書き方をすると可読性が犠牲になります。この問題を解決するときに使うのが、パイプライン処理のパッケージです。
...
@chainマクロを使うと、そもそもパイプライン演算子``|>``を記述する必要がなくなります。
最初の例は、次のように一行で書けます。
```julia
using Chain
@chain 1 tan cos sin
...
@chain 1 add(2) add(3)
```
左側の計算結果を次の関数の最初に引数として扱ってくれるからです。
この@chainマクロが何をしているか展開して確認してみましょう。
```!
using Chain
